    ...
    <form action="{{ route('admin.customers.store') }}" method="POST">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 1rem;">
                        <label style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.5rem; background: var(--light); border-radius: 8px; cursor: pointer; border: 2px solid var(--border);" onclick="toggleCustomerType('individual')">
                            <input type="radio" name="type" value="individual" {{ old('type', 'individual') === 'individual' ? 'checked' : '' }} required>
                            <div>
                        ...
                        <label style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.5rem; background: var(--light); border-radius: 8px; cursor: pointer; border: 2px solid var(--border);" onclick="toggleCustomerType('company')">
                            <input type="radio" name="type" value="company" {{ old('type') === 'company' ? 'checked' : '' }} required>
                            <div>
                ...
                <div id="individual-fields" style="display: {{ old('type', 'individual') === 'individual' ? 'block' : 'none' }};">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                        <div class="form-group">
                            <label class="form-label">Prénom</label>
                            <input type="text" name="first_name" class="form-input" placeholder="Mamadou" value="{{ old('first_name') }}">
                            @error('first_name')
                            <span style="color: var(--danger); font-size: 0.85rem;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="last_name" class="form-input" placeholder="Diallo" value="{{ old('last_name') }}">
                            @error('last_name')
                            <span style="color: var(--danger); font-size: 0.85rem;">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div id="company-fields" style="display: {{ old('type') === 'company' ? 'block' : 'none' }};">
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem;">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="company_contact_name" class="form-input" placeholder="Mamadou Diallo" value="{{ old('company_contact_name') }}">
