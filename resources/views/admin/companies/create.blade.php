...
<div class="content">
    <div class="page-header">
        <div>
            <h1 class="page-title">Créer une Entreprise</h1>
            ...
            <div class="card-body">
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
                    ...
                    <div class="form-group" style="grid-column:span 2;">
                        ...
                        <input type="text" name="raison_sociale" class="form-input" placeholder="Diallo & Frères SARL" value="{{ old('raison_sociale') }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="registration_number" class="form-input" placeholder="RC-2024-001" value="{{ old('registration_number') }}" required>
                        @error('registration_number')<span style="color:var(--danger);font-size:.85rem;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Email</label>
                        <input type="email" name="email" class="form-input" placeholder="contact@entreprise.com" value="{{ old('email') }}" required>
                        @error('email')<span style="color:var(--danger);font-size:.85rem;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Téléphone</label>
                        <input type="text" name="phone" class="form-input" placeholder="+224 XXX XX XX XX" value="{{ old('phone') }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="credit_limit" class="form-input" placeholder="10000000" value="{{ old('credit_limit') }}" step="0.01" min="0" required>
                        @error('credit_limit')<span style="color:var(--danger);font-size:.85rem;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group" style="grid-column:span 2;">
                        <label class="form-label">Adresse</label>
                        <input type="text" name="address" class="form-input" placeholder="Quartier, Rue, Immeuble" value="{{ old('address') }}" required>
                        @error('address')<span style="color:var(--danger);font-size:.85rem;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Ville</label>
                        <input type="text" name="city" class="form-input" placeholder="Conakry" value="{{ old('city') }}" required>
                        @error('city')<span style="color:var(--danger);font-size:.85rem;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Pays</label>
                        <input type="text" name="country" class="form-input" placeholder="Guinée" value="{{ old('country', 'Guinée') }}" required>
